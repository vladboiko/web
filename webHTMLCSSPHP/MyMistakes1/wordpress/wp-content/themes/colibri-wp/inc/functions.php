...
Hooks::colibri_do_action( 'theme_loaded', 'colibri_theme_loaded' );
...
add_action( 'wp_ajax_colibriwp_front_set_predesign', function () {
    ...
    foreach ( Defaults::get( 'front_page_designs', array() ) as $predesign ) {
        if ( intval( $predesign['index'] ) === $predesign_index ) {
            $meta = \ColibriWP\Theme\Core\Utils::pathGet( $predesign, 'meta', array() );
            break;
        }
    }
...
Hooks::colibri_add_action( 'theme_loaded', function () {
